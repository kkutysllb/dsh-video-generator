...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/* ── 工作台主面板：新建 hero + 作品库（与设置页职责分离）── */

		/**
		 * 工作台 = 任务导向的工作界面:上方是「新建」(题材一句话 → 创作提示词
		 * 一键填入当前对话输入框),下方是「作品库」(runs 列表/详情,3s 自动刷新)。
		 * 与设置页(通道/预算配置)完全分离。sendToChat 由 apply 注入(闭包持
		 * ctx),组件自身不碰宿主服务,便于测试。
		 */
		function makeWorkbenchComponent(t, sendToChat) {
			function Workbench() {
				...
				// 作品库：runs 列表 + 选中详情（复用设置页时代的 StudioView 渲染）
				...
				var onSend = function () {
					...
					Promise.resolve(sendToChat(fill(t("promptVgen"), { topic: text })))
						.then(setSent) // draft | clipboard | none
						.catch(function () { setSent("none"); });
				};

				return React.createElement("div", { className: "vg-root" },
					React.createElement("div", { className: "vg-work-hero" },
						React.createElement("h2", null, t("workTitle")),
						React.createElement("p", { className: "vg-work-intro" }, t("workIntro")),
						React.createElement("div", { className: "vg-field" },
							React.createElement("label", null, t("topicLabel")),
							React.createElement("textarea", {
								className: "vg-textarea", rows: 2,
								placeholder: t("topicPlaceholder"),
								value: topic,
								onChange: function (e) { setTopic(e.target.value); setSent(""); },
							}),
						),
						React.createElement("div", { className: "vg-work-cta" },
							Btn({ primary: true, onClick: onSend }, t("sendToChat")),
							sent === "copied" ? React.createElement("span", { className: "vg-work-sent" }, t("sentCopied")) : null,
							sent === "clipboard" ? React.createElement("span", { className: "vg-hint" }, t("sentClipboard")) : null,
							sent === "none" ? React.createElement("span", { className: "vg-hint" }, t("sendNone")) : null,
							sent === "required" ? React.createElement("span", { className: "vg-hint" }, t("topicRequired")) : null,
						),
					),
					React.createElement("div", { className: "vg-work-sec" },
						React.createElement("div", { className: "vg-row" },
							React.createElement("h3", null, t("worksTitle")),
							React.createElement("div", { style: { flex: 1 } }),
							React.createElement("span", { className: "vg-tpl-meta" }, t("autoRefreshHint")),
						),
						React.createElement(StudioView, {
							t: t,
							runs: runs, detail: detail, selected: selected,
							onSelectRun: function (id) { setSelected(id); },
							onBack: function () { setSelected(null); setDetailData(null); },
						}),
					),
				);
			}
		...
		function apply(ctx) {
			...
			var sendToChat = function (text) {
				...
				return Promise.resolve(write).then(function (result) {
					...
					try {
						...
						console.warn("dsh-video-generator sendToChat 会话桥异常:", bridgeError && bridgeError.message);
					}
					return "copied";
				});
			};;;;
			...
			// 图标行位于「新任务」与工作区列表之间；壳层拥有按钮/Tooltip/active
			// 态，注册方只出图标字形（场记板）。两段注册同 id 'vgen-panel'，
			// 面板体复用设置页 Stateful（工坊 run 列表 + 通道管理同一份数据面）。
			// 软探测：宿主 ≤0.1.4 无这些 slot 时静默跳过，设置页入口仍在。
			try {
				var PANEL_ID = "vgen-panel";
