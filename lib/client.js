...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
				...
				return h("div", { className: "vg-wb-card", onClick: function () { props.onOpen(p); } },
					h("div", { className: "vg-row", style: { alignItems: "flex-start", gap: 8 } },
						h("div", { style: { flex: 1, minWidth: 0 } },
							...
							h("div", { className: "vg-tpl-meta", style: { marginTop: 2 } }, p.category || "", p.language ? " · " + p.language : ""),
						),
			...
			function DramaApp() {
				...
				return h("div", { className: "vg-wb-page" },
					...
					!wizard && projects && list.length === 0 ? h("div", { className: "vg-wb-empty" },
						...
						h("button", { className: "vg-btn vg-btn-primary", style: { marginTop: 12 }, onClick: function () { setWizard(true); } }, "+ " + t("newProject")),
					) : null,
		...
		function apply(ctx) {
			...
			// ── 任务指令发送桥（§6.2）：预填会话输入框 → 切回会话视图，用户回车即发。
			// 预填走 uiConversation.fillDraft（宿主专用 API：SessionId 显式寻址、无需作用域
			// 注入；对尚未挂载输入壳的新会话是 no-op，故失败时安排短重试兜挂载时序）。
			// 剪贴板始终同步写入作为兜底（粘贴 ⌘V/Ctrl+V 后回车）。
			// 返回 Promise<{result:'prefilled'|'copied'|'none', sessionId?}>。
			var prefillComposer = function (sessionId, text) {
				try {
					if (!sessionId) return false;
					var ui = ctx.uiConversation;
					if (!ui || typeof ui.fillDraft !== "function") return false;
					ui.fillDraft(sessionId, text);
					return true;
			...
			var sendInstruction = function (text) {
				...
				return Promise.resolve(write).then(function (result) {
					...
					return ensure.then(function (sid) {
						...
						var target = sid || current;
						var immediate = prefillComposer(target, text);
						if (!immediate) {
							// 新会话输入壳可能未挂载：延迟重试（fillDraft 幂等，重复写无害）
							setTimeout(function () { prefillComposer(target, text); }, 600);
							setTimeout(function () { prefillComposer(target, text); }, 1800);
						}
						if (immediate) return { result: "prefilled", sessionId: target };
						if (result !== "copied") return { result: "none", sessionId: target };
						return { result: "copied", sessionId: target };
					});
				});
			};
