...
test('updateChannel 只改提供的字段且永不明文回显；delete 生效', () => {
  ...
  try {
    const store = VaultStore.open({ file: join(home, 'vault.json') })
    store.createChannel({ ...GOOD, models: [{ model: 'old-image', kind: 'image' }, { model: 'old-video', kind: 'video' }] })
    const updated = store.updateChannel('vectorengine', { label: 'VE', enabled: false, baseUrl: 'https://api.vectorengine.cn/v1', models: [] })
    ...
    assert.deepEqual(store.getChannel('vectorengine')?.models, [])
